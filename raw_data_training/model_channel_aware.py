...
class ChannelAwareEEGNet(nn.Module):
    """
    通道角色感知的EEG网络
    ...
    def forward(self, x, channel_mask=None):
        """
        Args:
            x: (batch, n_channels, n_samples) EEG数据
        ...
        if channel_mask is None:
            channel_mask = torch.ones(batch_size, self.n_channels, device=x.device)
        
        # 1. 时间卷积特征提取
        temporal_features = self.temporal_conv(x)  # (batch, 256, seq_len)
...
if __name__ == "__main__":
    ...
    model = create_channel_aware_model(n_channels, n_samples, n_classes)
