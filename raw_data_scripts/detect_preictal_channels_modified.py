...
# -*- coding: utf-8 -*-
"""
detect_preictal_channels_modified.py

This is a modified version of detect_preictal_channels.py.

Modifications:
1. Clarified the concept of channel abnormality: A channel is considered abnormal in a time segment if its features deviate significantly from its own baseline patterns and exhibit inconsistencies compared to other channels, potentially indicating the initial source of epileptic seizure signals.
2. Added support to read doctor-annotated important abnormal channels from 'effect_channel.csv' in the same directory as the input .set file. If present, only these channels are analyzed; otherwise, falls back to default channels.

Purpose: Detect "earliest abnormal" electrodes (channels) in pre-ictal phase and mark their abnormal time intervals.
Applicable: Scalp EEG, based on user-provided baseline segments and seizure onset time.

Usage example:
    python detect_preictal_channels_modified.py --input patient1.set --seizure_onset 3600 \
        --baseline 0 300 --preictal_minutes 30 --out result.json

Dependencies:
    pip install mne numpy scipy pandas matplotlib plotly
"""
...
# ---------------------------
# Utility functions - Feature computation
# ---------------------------
def spectral_entropy(sig, n_bins=50):
    """Compute (amplitude) entropy of signal using histogram approximation of PDF"""
    if np.all(sig == 0):
        return 0.0
...
def bandpower(sig, sfreq, band, method='welch'):
    """Simplified band power: directly approximate via time-domain variance (for short windows) or extend to PSD.
    Here, use variance as power approximation for speed; can replace with PSD computation"""
    if method == 'welch':
...
def line_length(sig):
    """Line length (often used for burst/non-stationary detection)"""
    return float(np.sum(np.abs(np.diff(sig))))


def compute_features_for_segment(sig, sfreq):
    """Compute feature vector for a segment (1D np.array)"""
    features = {
...
# ---------------------------
# Time segment and sliding window utilities
# ---------------------------
...
def merge_close_intervals(intervals, max_gap):
    """Merge intervals that are close to each other; intervals: list of (start, end) sorted"""
    if not intervals:
        return []
    merged = []
...
def benjamini_hochberg(pvals, alpha=0.05):
    """Return boolean list: whether to reject null hypothesis (i.e., significant)"""
    p = np.array(pvals)
    n = len(p)
    if n == 0:
...
def detect_preictal_channels(raw,
    ...
    Abnormality Concept Clarification:
    - A channel is flagged as abnormal in a specific time segment if its computed features (e.g., variance, band powers) show significant deviation (z-score > threshold) from its own baseline distribution.
    - Additionally, this deviation should indicate inconsistency with other channels' behavior in the same segment, meaning the channel's z-score is notably higher than the average across other channels, suggesting it may be the initial source of epileptic signals.
    - To incorporate cross-channel inconsistency, we add a check: for a window to be abnormal, the channel's max_z must exceed the mean max_z of other channels by an additional threshold (e.g., 1.5 * std of others).

    Returns: dict results and saves JSON + images
    """
    ...
    # Define baseline data (concatenate multiple baseline intervals)
    baseline_data_list = []
    ...
    # 1) Baseline feature distributions (per channel, sliding windows)
    ...
    # 2) Compute features and z-scores in pre-ictal sliding windows
    ...
    # 3) Determine if each window is abnormal (any feature z > z_thr, plus cross-channel check)
    per_ch_win_flag = {}
    per_ch_maxz = {}
    all_maxz = np.zeros((len(pick_chs), len(windows)))  # For cross-channel comparison
    for ch_idx, ch in enumerate(pick_chs):
        zmat = per_ch_z[ch]  # n_windows x features
        maxz = np.max(zmat, axis=1)  # per-window max z
        all_maxz[ch_idx] = maxz
        # Cross-channel inconsistency: for each window, check if this channel's maxz > mean(other) + 1.5*std(other)
        flags = np.zeros(len(maxz), dtype=bool)
        for w in range(len(maxz)):
            other_maxz = np.delete(all_maxz[:, w], ch_idx)
            if len(other_maxz) > 0:
                other_mean = np.mean(other_maxz)
                other_std = np.std(other_maxz) if len(other_maxz) > 1 else 0
                inconsistency = maxz[w] > (other_mean + 1.5 * other_std)
            else:
                inconsistency = True  # If no other channels, skip check
            flags[w] = (maxz[w] > z_thr) and inconsistency
        per_ch_win_flag[ch] = flags
        per_ch_maxz[ch] = maxz

    # 4) Convert consecutive abnormal windows to time intervals (require at least min_consec)
    results = []
    per_ch_pval = []
    per_ch_interval_z_avgs = {ch: [] for ch in pick_chs}  # New: per-channel z avgs
    for ch in pick_chs:
        ...
        if intervals_sorted:
            ...
            for (ms, me) in merged:
                ...
                for idx, (s, e, sc) in enumerate(intervals_sorted):
                    samp_s = int((s - preictal_t0) * sfreq)
                    samp_e = int((e - preictal_t0) * sfreq)
                    if not (samp_e <= ms or samp_s >= me):
                        contrib_scores.append(sc)
                        win_start = windows[idx]  # Note: assuming idx corresponds to window index; adjust if needed
                        win_end = win_start + length  # length from above; ensure correct
                        for feat in feature_names:
                ...
                per_ch_interval_z_avgs[ch].append(avg_z)  # Fill per-channel list
        ...
        ch_results[ch] = {
            ...
            'p_value': float(pval),
            'interval_z_avgs': per_ch_interval_z_avgs[ch]  # New: store per-channel z avgs
        }

    # 5) Multiple comparison correction (Benjamini-Hochberg) using pseudo-pvals
    pvals = per_ch_pval
    ...
    for entry in final_list_sorted:
        ch = entry['channel']
        interval_z_avgs = ch_results[ch].get('interval_z_avgs', [])  # Use per-channel list
        for int_idx, interval in enumerate(entry['intervals']):
            ...
            if int_idx < len(interval_z_avgs):
                ...
                abnormal_bands = ''  # Edge case: no z data
...
# ---------------------------
def parse_args():
    p = argparse.ArgumentParser(description="Detect earliest pre-ictal significant channels (modified)")
    ...
    p.add_argument('--channels', nargs='+', default=DEFAULT_CHANNELS, help="Channel list to analyze (overridden by effect_channel.csv if present)")
...
def main():
    ...
    # Check for effect_channel.csv in the same directory as input
    input_dir = os.path.dirname(args.input)
    effect_csv_path = os.path.join(input_dir, 'effect_channel.csv')
    if os.path.exists(effect_csv_path):
        print(f"Found effect_channel.csv at {effect_csv_path}. Loading doctor-annotated channels.")
        with open(effect_csv_path, 'r') as f:
            reader = csv.reader(f)
            annotated_channels = [row[0].strip() for row in reader if row]  # Assume first column is channel name
        args.channels = annotated_channels
    else:
        print(f"No effect_channel.csv found at {effect_csv_path}. Using default or provided channels.")
...
def gui_main():
    def run_analysis():
        try:
            ...
            if os.path.exists(effect_csv_path):
                ...
                with open(effect_csv_path, 'r') as f:
                    reader = csv.reader(f)
                    annotated_channels = [row[0].strip() for row in reader if row]
                channels = annotated_channels
            else:
                print(f"No effect_channel.csv found. Using provided channels.")
    ...
    root = tk.Tk()
    root.title("Preictal Channel Detector (Modified)")
