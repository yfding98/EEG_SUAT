...
# 导入频段特定模块
from frequency_band_dataset import create_frequency_band_dataloaders
from frequency_band_model import (
    create_frequency_band_classifier,
    FrequencyBandLoss,
    compute_frequency_band_metrics
)
...
class FrequencyBandTrainer:
    ...
    def train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        pbar = tqdm(self.train_loader, desc=f"Epoch {epoch} [{self.frequency_band}] [Train]")
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            # 前向传播
            logits = self.model(data)  # (batch, n_channels)
            ...
            # 计算指标
            with torch.no_grad():
                metrics = compute_frequency_band_metrics(logits.detach(), labels.detach())
    ...
    def validate(self, epoch: int, phase: str = 'Val') -> Dict[str, float]:
        ...
        pbar = tqdm(loader, desc=f"Epoch {epoch} [{self.frequency_band}] [{phase}]")
        for batch in pbar:
            ...
            # 计算指标
            metrics = compute_frequency_band_metrics(logits, labels)
    ...
    def train(self, n_epochs: int) -> Tuple[float, float]:
        """训练主循环"""
        print(f"\n{'='*80}")
        print(f"开始训练 {self.frequency_band} 频段分类器")
        print(f"{'='*80}")
        ...
        print(f"\n{'='*80}")
        print(f"{self.frequency_band} 频段测试集结果")
        print(f"{'='*80}")
...
def main():
    parser = argparse.ArgumentParser(description='频段特定分类器训练')
    
    # 数据参数
    parser.add_argument('--data_root', type=str, required=True,
                        help='数据根目录')
    parser.add_argument('--frequency_band', type=str, required=True,
    ...
    save_dir = Path(args.save_dir) / f"{args.frequency_band}_{timestamp}"
    ...
    try:
        train_loader, val_loader, test_loader, channel_names = create_frequency_band_dataloaders(
    ...
    # 创建模型
    print(f"\n创建 {args.frequency_band} 频段分类器...")
    model = create_frequency_band_classifier(
    ...
    print(f"  频段: {args.frequency_band}")
    
    # 损失函数
    criterion = FrequencyBandLoss(
        frequency_band=args.frequency_band,
        alpha=args.focal_alpha,
        gamma=args.focal_gamma if args.use_focal_loss else 0.0,
        use_class_weights=args.use_class_weights
    )
    print(f"使用频段特定损失函数 ({args.frequency_band})")
    ...
    print(f"\n{'='*80}")
    print(f"{args.frequency_band} 频段训练完成!")
    print(f"{'='*80}")
